...
rectangle_check_collision :: proc(rec1, rec2: common.Rectangle) -> bool {
	return(
...
Game :: struct {
	ground:        [4]common.Rectangle,
	pipes:         [4]Pipe,
	player:        Player,
	background:    [3]common.Point,
	current_state: GameState,
}

Pipe :: struct {
	upper_hitbox: common.Rectangle,
	score_hitbox: common.Rectangle,
	lower_hitbox: common.Rectangle,
	pos_x, pos_y: f32,
	scored:       bool,
}
...
pipes_init :: proc(pipes: []Pipe) {
	for i: i32 = 0; i < i32(len(pipes)); i += 1 {
		...
		pipes[i] = Pipe {
			pos_x        = x,
			pos_y        = magic_y,
			upper_hitbox = common.Rectangle{x, magic_y, PIPE_WIDTH, PIPE_HEIGHT},
			score_hitbox = common.Rectangle {
				x,
				magic_y + PIPE_HEIGHT,
				PIPE_WIDTH,
...
ground_init :: proc(ground: []common.Rectangle) {
	ground_height :: 112
...
ground_update :: proc(ground: []common.Rectangle) {
...
background_init :: proc(background: []common.Point) {
	for i := 0; i < len(background); i += 1 {
		background[i] = common.Point {
...
background_update :: proc(background: []common.Point) {
...
player_init :: proc(player: ^Player) {
	player.hitbox = common.Rectangle {
		x      = 100.0,
	...
	player.velocity_y = 0
	player.highest_score = platform.read_highest_score()
}
...
player_apply_gravity :: proc(player: ^Player, ground: []common.Rectangle) {
	for ground_piece in ground {
...
game_playing :: proc() {
	...
	case .JUMP:
		...
		platform.sound_play(common.Sound.JUMP_SOUND)
	case .PAUSE:
	...
	for &pipe in game.pipes {
		if rectangle_check_collision(game.player.hitbox, pipe.score_hitbox) && !pipe.scored {
			game.player.score += 1
			pipe.scored = true
			platform.sound_play(common.Sound.SCORE_SOUND)
			//PIPE_VELOCITY += 10 :D
		}
		when GODMODE == false {
			if rectangle_check_collision(game.player.hitbox, pipe.lower_hitbox) ||
			   rectangle_check_collision(game.player.hitbox, pipe.upper_hitbox) {
				...
				platform.sound_play(common.Sound.DEATH_SOUND)
			}
		}
	}
	when GODMODE == false {
		for ground_piece in game.ground {
			if rectangle_check_collision(game.player.hitbox, ground_piece) {
				...
				game.player.hitbox.y =
					f32(WINDOW_HEIGHT) - ground_piece.height - game.player.hitbox.height
				platform.sound_play(common.Sound.DEATH_SOUND)
			}
		}
	}
...
update :: proc() {
	...
	case .GAME_OVER:
		if game.player.score > game.player.highest_score {
			...
			platform.save_highest_score(game.player.score)
		}
		game_over()
	case .GAME_PAUSE:
		game_pause()
